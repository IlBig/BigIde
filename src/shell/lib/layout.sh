...
create_layout() {
  ...
  local top_pane_id left_top_id right_top_id left_bottom_id terminal_pane_id
  ...
  # Split orizzontale: sinistra (file-tree+monitor, stretta) | destra (claude+terminal, larga)
  right_top_id="$(tmux split-window -h -p "$claude_width" -t "$top_pane_id" -P -F '#{pane_id}')"
  left_top_id="$top_pane_id"

  # Split verticale sinistra: file tree (alto) | monitor (basso)
