...
export const watchPaneTool = {
    ...
    handler: async ({ target, timeoutMs, pollIntervalMs, captureLines }) => {
        try {
            ...
            while (Date.now() - start < timeoutMs) {
                ...
                if (current !== initial) {
                    ...
                    return {
                        content: [{ type: "text", text: newContent || current }]
                    };
                }
            }
            return {
                content: [{ type: "text", text: "No changes detected within timeout." }]
            };
        }
        catch (error) {
