...
export const createPaneTool = {
  ...
  handler: async ({ target, direction, size, command }: any) => {
    try {
      ...
      return {
        content: [{ type: "text" as const, text: `Created pane ${newPaneId}` }]
      };
    } catch (error: any) {
...
export const closePaneTool = {
  ...
  handler: async ({ target }: any) => {
    try {
      logger.info(`Closing pane ${target}`);
      client.killPane(target);
      return {
        content: [{ type: "text" as const, text: `Closed pane ${target}` }]
      };
    } catch (error: any) {
...
export const resizePaneTool = {
  ...
  handler: async ({ target, direction, amount }: any) => {
    try {
      logger.info(`Resizing pane ${target} ${direction} ${amount}`);
      client.resizePane(target, direction, amount);
      return {
        content: [{ type: "text" as const, text: `Resized pane ${target}` }]
      };
    } catch (error: any) {
